...
1. Clone the CodeGeass source code:
    ```shell
    git clone https://github.com/CodeGeass/codegeass
    cd codegeass
    ```

2.  Build CodeGeass:

    Configure and build the headless CodeGeass binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, CodeGeass may be
...
## Running
CodeGeass is now available at `./src/codegeassd`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/CodeGeass"

touch "/Users/${USER}/Library/Application Support/CodeGeass/codegeass.conf"

chmod 600 "/Users/${USER}/Library/Application Support/CodeGeass/codegeass.conf"
```

The first time you run codegeassd, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/CodeGeass/debug.log
```

## Other commands:
```shell
./src/codegeassd -daemon      # Starts the codegeass daemon.
./src/codegeass-cli --help    # Outputs a list of command-line options.
./src/codegeass-cli help      # Outputs a list of RPC commands when the daemon is running.
```
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/CodeGeass/codegeass-detached-sigs).
